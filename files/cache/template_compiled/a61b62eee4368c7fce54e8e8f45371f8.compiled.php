...
<?php if($__Context->XE_VALIDATOR_MESSAGE && $__Context->XE_VALIDATOR_ID == 'modules/document/tpl/declared_list/1'){ ?><div class="message <?php echo escape($__Context->XE_VALIDATOR_MESSAGE_TYPE, false) ?>">
...
<form id="fo_list" action="./" method="get"><input type="hidden" name="error_return_url" value="<?php echo htmlspecialchars(getRequestUriByServerEnviroment(), ENT_COMPAT | ENT_HTML401, 'UTF-8', false) ?>" /><input type="hidden" name="act" value="<?php echo $__Context->act ?>" /><input type="hidden" name="mid" value="<?php echo $__Context->mid ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" />
	...
	<table class="x_table x_table-striped x_table-hover" id="documentListTable">
		<caption>
			<a href="<?php echo escape(getUrl('', 'module', 'admin', 'act', 'dispDocumentAdminList'), false) ?>"><?php echo $__Context->lang->all ?></a>
			<i>|</i>
			<a href="<?php echo escape(getUrl('act', 'dispDocumentAdminList', 'search_target', 'is_secret', 'search_keyword', 'N'), false) ?>"><?php echo escape($__Context->status_name_list['PUBLIC'], false) ?></a>
			<i>|</i>
			<a href="<?php echo escape(getUrl('act', 'dispDocumentAdminList', 'search_target', 'is_secret', 'search_keyword', 'Y'), false) ?>"><?php echo escape($__Context->status_name_list['SECRET'], false) ?></a>
			<i>|</i>
			<a href="<?php echo escape(getUrl('act', 'dispDocumentAdminList', 'search_target', 'is_secret', 'search_keyword', 'temp'), false) ?>"><?php echo escape($__Context->status_name_list['TEMP'], false) ?></a> 
			...
			<i>|</i>
			<a href="<?php echo escape(getUrl('', 'module', 'admin', 'act', 'dispDocumentAdminDeclared'), false) ?>"<?php if($__Context->act == 'dispDocumentAdminDeclared'){ ?> class="active"<?php } ?>><?php echo $__Context->lang->cmd_declared_list ?>(<?php echo escape(number_format($__Context->total_count), false) ?>)</a>
		...
		<tbody>
			<?php if($__Context->document_list&&count($__Context->document_list))foreach($__Context->document_list as $__Context->no=>$__Context->oDocument){ ?><tr>
				<td class="title"><a href="<?php echo escape(getUrl('','document_srl',$__Context->oDocument->document_srl), false) ?>" target="_blank"><?php echo $__Context->oDocument->getTitle() ?></a></td>
				...
				<td class="nowr"><?php echo escape($__Context->oDocument->get('declared_count'), false) ?></td>
...
<form action="./" method="post" class="x_modal" id="manageForm"><input type="hidden" name="error_return_url" value="<?php echo htmlspecialchars(getRequestUriByServerEnviroment(), ENT_COMPAT | ENT_HTML401, 'UTF-8', false) ?>" /><input type="hidden" name="mid" value="<?php echo $__Context->mid ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" />
	...
	<input type="hidden" name="success_return_url" value="<?php echo escape(getUrl('', 'module', 'admin', 'act', 'dispDocumentAdminDeclared'), false) ?>" />
