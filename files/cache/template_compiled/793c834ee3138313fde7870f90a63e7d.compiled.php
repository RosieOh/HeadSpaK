...
if($__Context->mi->default_style=='blog'){ ?> rd_blog <?php echo $__Context->mi->blog_style;
	...
	<div class="rd_hd clear" style="<?php if(!$__Context->mi->rd_style){ ?>margin:0 0 10px<?php };
if($__Context->mi->rd_padding){ ?>;margin-left:0;margin-right:0<?php } ?>">
		...
		<?php if(!$__Context->mi->rd_style){ ?><div class="board clear <?php echo $__Context->mi->rd_board_style ?>" style="<?php echo $__Context->mi->rd_css ?>;">
			<div class="top_area <?php echo $__Context->mi->rd_tl_font ?>" style="<?php echo $__Context->mi->rd_tl_css ?>;">
				...
				<h1 class="np_18px"><?php echo $__Context->oDocument->getTitle() ?></h1>
			</div>
		</div><?php } ?>
...
$__tpl=TemplateHandler::getInstance();echo $__tpl->compile('modules/board/skins/market_search_foreign','_read_files.html');
} ?>
...
$__tpl=TemplateHandler::getInstance();echo $__tpl->compile('modules/board/skins/market_search_foreign','_read_sns.html');
} ?>
...
$__tpl=TemplateHandler::getInstance();echo $__tpl->compile('modules/board/skins/market_search_foreign','_read_nav.html');
		...
		<?php if(!$__Context->mi->rd_nav_side){ ?><div class="rd_nav_side">
			<?php $__tpl=TemplateHandler::getInstance();echo $__tpl->compile('modules/board/skins/market_search_foreign','_read_nav.html') ?>
		</div><?php } ?>
		<?php } ?>
	</div>
...
if(in_array(strtolower(substr(strrchr($__Context->file->source_filename, '.'), 1)), array('jpg','jpeg','png','gif','webp'))){ ?><div class="swiper-slide rd_sli">
						<div class="rd_img" style="background-image:url(<?php echo $__Context->file->uploaded_filename ?>)"></div>
					</div><?php }} ?>
				</div>
		...
		<?php $__tpl=TemplateHandler::getInstance();echo $__tpl->compile('modules/board/skins/market_search_foreign','extravar_table.html') ?>
...
<?php } ?>
	...
	<?php if($__Context->oDocument->getExtraEidValueHTML('address')){ ?><div class="rd_map">
		<div class="rd_map_box">
			<?php $__Context->addr_content = $__Context->oDocument->getExtraEidValue('address'); $__Context->addr_content = htmlspecialchars($__Context->addr_content); ?>
			...
			<script>
				(function() {
					...
					geocoder.addressSearch("<?php echo $__Context->addr_content ?>", function(result, status) {
	...
	<?php if($__Context->oDocument->getExtraEidValueHTML('address')){ ?><div class="rd_map">
		...
		<script src="https://maps.googleapis.com/maps/api/js?key=AIzaSyBIV_FxXqVU1wl5Ol8RxyqRahRhyb97VmE&callback=initMap&libraries=&v=weekly" defer ></script>
		<script>
			"use strict";
			function initMap() {
				const map = new google.maps.Map(document.getElementById("rd_map_box"), {
					zoom: 16,
					center: {
						lat: -34.397,
						lng: 150.644
			...
			function geocodeAddress(geocoder, resultsMap) {
				...
				geocoder.geocode(
					{
						address: '<?php echo $__Context->addr_content ?>'
					},
					(results, status) => {
						if (status === "OK") {
		...
		<div class="rd_map_box" id="rd_map_box"></div>
	</div><?php } ?>
	<?php } ?>
...
$__tpl=TemplateHandler::getInstance();echo $__tpl->compile('modules/board/skins/market_search_foreign','_comment_write.html');
} ?>
			
			<div id="cmtPosition" aria-live="polite"><?php $__tpl=TemplateHandler::getInstance();echo $__tpl->compile('modules/board/skins/market_search_foreign','_comment.html') ?></div>
