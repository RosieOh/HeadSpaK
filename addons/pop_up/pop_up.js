	...
	jQuery.fn.xe_popup = function(options) {
		...
		return this.each(function() {
			...
			if (!getCookie(opts.id) || getCookie(opts.id) != "no"){
				...
				var popupclosertext = jQuery("<div style='padding-top:8px; overflow:hidden; border:0px red solid; width:"+ (parseInt(opts.width) - 61)+"px; height:33px; display: -moz-inline-stack; display: inline-block; zoom: 1; *display: inline; vertical-align:top;'>"+ 1 + "일 동안 다시보지 않기"+"</div>");
				...
				popupclosercheck.click(function(){setCookie(opts.id, "no", 1); jQuerythis.hide("fast");});
			...
			function setCookie( id, value, exp_days )
			{
				 ...
				 document.cookie = id + "=" + escape(value) + "; expires=" + todayDate.toUTCString();
				 //document.cookie = escape(id) + "=" + escape(value) + "; expires=" + todayDate.toUTCString();
